...
public record AddSubchannelTagFluidPacket(BlockPos emitterPos, UUID subId, ResourceLocation tagId) {
    ...
    public static AddSubchannelTagFluidPacket decode(FriendlyByteBuf b) {
        return new AddSubchannelTagFluidPacket(b.readBlockPos(), b.readUUID(), b.readResourceLocation());
    }
    public static void handle(AddSubchannelTagFluidPacket p, Supplier<NetworkEvent.Context> sup) {
        ...
        ctx.enqueueWork(() -> {
            ...
            if (em.addFluidSubchannelTagEntry(p.subId, p.tagId)) {
